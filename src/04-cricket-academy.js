...
export class Batsman extends Player {
  ...
  playInnings(runs, balls) {
    ...
    const newInnings = { runs, balls, strikeRate };
  ...
  getStrikeRate() {
    ...
    for (const inning of this.innings) {
      sumStrike += inning.strikeRate;
    }

    return sumStrike / this.innings.length;
  }

  getProfile() {
    const totalRuns = this.innings.reduce(
      (acc, inning) => acc + inning.runs,
      0,
    );
    return {
      ...super.getProfile(),
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const economySum = this.spells.reduce((acc, curr) => acc + curr.economy, 0);
    return economySum / this.spells.length;
  }

  getProfile() {
    const totalWickets = this.spells.reduce(
      (acc, curr) => curr.wickets + acc,
      0,
    );
...
export class AllRounder extends Player {
  ...
  playInnings(runs, balls) {
    ...
    const newInnings = { runs, balls, strikeRates };
    this.innings.push(newInnings);
    return newInnings;
  }
